...
###############################################################################
addResult() {
  # modified to accept args in a different fashion
  RESULT="<item uid=\"$(xmlEncode "$1")\" valid=\"$6\" autocomplete=\"$7\"><arg>$(xmlEncode "$2")</arg><title>$(xmlEncode "$3")</title><subtitle>$(xmlEncode "$4")</subtitle>"
  if [[ $5 =~ fileicon:* ]]; then
...
#  fi
  ...
  for R in ${RESULTS[*]}; do
    # hack to get newlines and asterisks to work.
    # use __N to escape
    # use __* for asterisk
    newResult=`echo "$R" | tr "\n" " " | sed 's/__N/\'$'\n/' | sed 's/__\*/*/'`
    echo -n "$newResult"
